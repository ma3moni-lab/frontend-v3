...
import { adminApi, blog as blogApi, analytics as analyticsApi, restoreAdminToken, type AnalyticsOverview, type PlatformSettings, type BlogArticle, type BlogCategory } from "../../lib/api";
...
function BlogEditorModal({ article, categories, onClose, onSaved }: {
  ...
  // Parse existing blocks from article.content (JSON) or start fresh
  ...
  const handleSave = async (publish: boolean) => {
    ...
    try {
      ...
      if (isEdit && article) {
        savedArticle = await blogApi.updateArticle(article.id, {
          title:       form.title,
        ...
        savedArticle = await blogApi.createArticle({
          title:       form.title,
          excerpt:     form.excerpt,
          content:     contentJson,
          category_id: form.categoryId || undefined,
      ...
      if (coverFile) {
        await blogApi.uploadCover(savedArticle.id, coverFile).catch(() => {});
      }
      if (publish && !isEdit) {
        await blogApi.publishArticle(savedArticle.id).catch(() => {});
      }
      ...
      <div className="bg-card rounded-2xl border border-border w-full max-w-3xl shadow-2xl my-6">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex gap-3 pt-2 border-t border-border">
            <button onClick={onClose} disabled={saving} className="px-5 py-3 rounded-xl border border-border hover:bg-muted transition-colors disabled:opacity-50" style={{ fontSize: "0.9rem" }}>
              Cancel
            </button>
            {!isEdit && (
      ...
      {tab === "categories" && (
        <div className="space-y-4">
          ...
          <div className="bg-card rounded-2xl border border-border overflow-hidden">
            ...
            {categories.length === 0 && (
              <div className="px-5 py-8 text-center text-muted-foreground" style={{ fontSize: "0.875rem" }}>No categories yet.</div>
            )}
            {categories.map(cat => {
              ...
              <div className="flex items-center gap-3 pt-4 border-t border-border">
                <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center">
                  <span style={{ fontWeight: 800, fontSize: "0.75rem", color: "var(--primary)" }}>
                    {(preview.author?.full_name ?? "?").split(" ").map(n => n[0]).join("").slice(0, 2).toUpperCase()}
                  </span>
                </div>
                <div>
