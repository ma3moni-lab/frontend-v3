        ...
        <button
          onClick={async () => {
            try {
              ...
              const photos: {image_url: string}[] = data.photos ?? [];
              const now = new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
              const field = (label: string, val: unknown) =>
                val ? `<tr><td style="color:#555;padding:4px 12px 4px 0;font-size:13px;white-space:nowrap">${label}</td><td style="padding:4px 0;font-size:13px">${val}</td></tr>` : '';
              const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Ma3moni Profile — ${p.full_name ?? a.email}</title>
<style>
  body{font-family:'Segoe UI',Arial,sans-serif;color:#111;margin:0;padding:32px 40px;max-width:760px;margin:0 auto}
  h1{font-size:22px;color:#0A6870;margin:0 0 2px}
  .sub{color:#666;font-size:13px;margin-bottom:24px}
  h2{font-size:14px;text-transform:uppercase;letter-spacing:.05em;color:#0A6870;border-bottom:1.5px solid #e0e0e0;padding-bottom:6px;margin:24px 0 10px}
  table{width:100%;border-collapse:collapse}
  .photos{display:flex;flex-wrap:wrap;gap:8px;margin-top:8px}
  .photos img{width:100px;height:120px;object-fit:cover;border-radius:6px}
  @media print{body{padding:20px}.no-print{display:none}}
</style></head><body>
<button class="no-print" onclick="window.print()" style="background:#0A6870;color:white;border:none;padding:8px 20px;border-radius:6px;cursor:pointer;font-size:13px;margin-bottom:20px">Save as PDF</button>
<h1>${p.full_name ?? 'Ma3moni User'}</h1>
<div class="sub">Profile export generated on ${now}</div>
<h2>Account</h2>
<table>${field('Email', a.email)}${field('Plan', a.plan)}${field('Joined', a.date_joined ? new Date(a.date_joined).toLocaleDateString('en-GB') : '')}${field('Verified', a.is_verified ? 'Yes' : 'No')}</table>
<h2>Personal Info</h2>
<table>${field('Full name', p.full_name)}${field('Gender', p.gender)}${field('Date of birth', p.date_of_birth)}${field('Nationality', p.nationality)}${field('Ethnicity', p.ethnicity)}${field('Blood group', p.blood_group)}${field('Genotype', p.genotype)}${field('City', p.location_city)}${field('Country', p.location_country)}${field('Marital status', p.marital_status)}</table>
<h2>Faith &amp; Lifestyle</h2>
<table>${field('Sect', p.sect)}${field('Prayer frequency', p.prayer_frequency)}${field('Quran recitation', p.quran_recitation)}${field('Hijab preference', p.hijab_preference)}${field('Smoking', p.smoking)}${field('Drinking', p.drinking)}</table>
<h2>Career &amp; Education</h2>
<table>${field('Profession', p.profession)}${field('Education', p.education)}${field('Income range', p.income_range)}${field('Living situation', p.living_situation)}</table>
<h2>About Me</h2>
<p style="font-size:13px;line-height:1.6;white-space:pre-wrap">${p.bio ?? '—'}</p>
<h2>Partner Preferences</h2>
<table>${field('Age range', p.pref_age_min || p.pref_age_max ? `${p.pref_age_min ?? '—'} – ${p.pref_age_max ?? '—'} years` : '')}${field('Preferred location', p.pref_location)}${field('Preferred nationality', p.pref_nationality)}${field('Children pref', p.pref_children)}${field('Relocation', p.pref_relocation)}</table>
${photos.length ? `<h2>Photos (${photos.length})</h2><div class="photos">${photos.map(ph => `<img src="${ph.image_url}" />`).join('')}</div>` : ''}
<p style="font-size:11px;color:#aaa;margin-top:40px;border-top:1px solid #eee;padding-top:12px">This document was generated by Ma3moni. All rights reserved.</p>
</body></html>`;
              const win = window.open('', '_blank');
              if (win) { win.document.write(html); win.document.close(); }
              else toast.success('Your data export is ready.');
            } catch {
          ...
          <div className="text-left">
            ...
            <p className="text-muted-foreground" style={{ fontSize: "0.8125rem" }}>Export a PDF summary of your profile &amp; activity</p>
          </div>
